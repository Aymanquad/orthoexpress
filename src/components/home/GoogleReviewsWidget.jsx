...
import { CLINIC, PATIENT_REVIEWS } from '../../data'
import { useLanguage } from '../../context/LanguageContext'
import './GoogleReviewsWidget.css'

const GoogleLogo = () => (
  <svg className="google-logo" viewBox="0 0 24 24" aria-hidden="true" width="20" height="20">
...
const GoogleReviewsWidget = () => {
  ...
  const { rating, count, mapsUrl } = CLINIC.googleReviews
  const reviewCountLabel = t('reviews.count').replace('{count}', String(count))
          ...
          <article key={review.name} className="google-review-card">
            <div className="google-review-card-header">
              ...
              <div>
                ...
                <span>{review.when[lang] || review.when.en}</span>
              </div>
              <GoogleLogo />
            </div>
            ...
            <p className="google-review-text">"{review.text[lang] || review.text.en}"</p>
          </article>
        ))}
      </div>
    </div>
